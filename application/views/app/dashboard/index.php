    ...
    <div class="col-12">
        <div class="card card-body gap-3">
            <div class="flex0">
                ...
                <div class="d-flex flex-column flex-md-row gap-3 datepicker-range0">
                    <div class="form-floating">
                        <input id="d1" class="form-control" type="text" value="<?= date('Y-m-01') ?>" onchange="tbl1()">
                        <label for="d1">Start:</label>
                    </div>
                    <div class="form-floating">
                        <input id="d2" class="form-control" type="text" value="<?= date('Y-m-t') ?>" onchange="tbl1()">
                        <label for="d2">End:</label>
                    </div>
                </div>
            </div>
            <table class="table table-sm table-hover tbl1">
                <thead>
                    <tr class="trth0">
                        <th class="w100px">PO No.</th>
                        <th class="">Vendor</th>
                        <th class="">Date Delivered</th>
                        <th class="">Date Encoded</th>
                        <th class="">Payment Mode</th>
                        <th class="w100px">Amount</th>
                    </tr>
                </thead>
                <tfoot>
                    <tr>
                        <th class="text-end" colspan="5">Total Amount:</th>
                        <th class="text-end tbl1-total_amount">0.00</th>
                    </tr>
                </tfoot>
            </table>
        </div>
...
<script>
    function purchase_paid(){
        return $.ajax({
            url: '<?= base_url() ?>ajax/purchase_paid',
            type: 'post',
            dataType: 'json',
        ...
        purchase_paid().done((res) => {
            $('.tbl1').DataTable({
                ...
                columns: [
                    {
                        data: 'id',
                        render: (d) => `<a href="#" onclick="po_view(${ d })">${ sprintf(6, d) }</a>`,
                    ...
                    },
                    {
                        ...
                        render: (d, t, row) => {
                            ...
                            payments.forEach(function(py){
                                py_details += (`<small>
                                    ${ py.payment_method_name ? py.payment_method_name + '<br>' : '' }
                                    ${ py.source ? py.source + '<br>' : '' }
                                    ${ py.reference ? py.reference + '<br>' : '' }
                                </small>`);
                            });
                            return '<small>' + py_details + '</small>';
                        }
                    },
                    {
                        data: 'total_amount',
                ...
                drawCallback: function(){
                    let api = this.api();
                    let total = api.column(3, { search: 'applied' }).data().reduce((a, b) => {
                        return (parseFloat(a) || 0) + (parseFloat(b) || 0);
                    }, 0);
                    $('.tbl1-total_amount').html(number_format(total, 2));
                }
            });
        });
    }
    $(() => { tbl1(); });
    async function po_view(id){
        const po = await r_tbl('purchase', false, { id: id });
        ...
        let po_paymentsTr = '';
        if(po){
            purchase_product = await r_tbl('purchase_product', true, { purchase_id: po.id });
            purchase_product.forEach((pp) => {
                const cost = parseFloat(pp.cost), qty = parseFloat(pp.qty), amount = cost * qty;
                purchase_productTr += (`<tr>
                    <td><input name="po-td-name" class="form-control input-view" type="text" value="${ pp.name }"></td>
                    <td><input name="po-td-barcode" class="form-control input-view" type="text" value="${ pp.barcode }"></td>
                    <td><input name="po-td-cost" class="form-control td-cost" type="text" value="${ cost }"></td>
                    <td><input name="po-td-qty" class="form-control td-qty" type="number" value="${ qty }"></td>
                    <td class="text-end td-amount">${ number_format(amount, 2) }</td>
                </tr>`);
                total_qty += qty; total_amount += amount;
            });
            po_payments = await r_tbl('purchase_payment', true, {purchase_id: po.id});
            po_payments.forEach((i) => {
                po_paymentsTr += (`<tr>
                    <td>${ date_MdY(i.paid_at) }</td>
                    <td><small>${ i.note }</small></td>
                    <td>${ i.payment_method_name }</td>
                    <td>
                        <small>
                            ${ i.source ? `Source: ${ i.source }<br>` : `` }
                            ${ i.reference ? `Ref. No.: ${ i.reference }<br>` : `` }
                        </small>
                    </td>
        ...
        modal0(
            ...
            ``,
            // head
            `<h5>Purchase Details <b>#${ sprintf(6, po.id) }</b></h5>`,
            // body
            `<div class="row g-3 to-view">
                <div class="col-lg-4">
                    <div class="form-floating">
                        <input id="po-vendor_name" class="form-control" type="text" value="${ po.vendor_name ?? '--' }">
                        <label for="po-vendor_name">Vendor Name</label>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="form-floating">
                        <input id="po-agent_name" class="form-control" type="text" value="${ po.agent_name ?? '--' }">
                        <label for="po-agent_name">Agent Name</label>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="form-floating">
                        <input id="po-vendor_contact_no" class="form-control" type="text" value="${ po.vendor_contact_no ?? '--' }">
                        <label for="po-vendor_contact_no">Contact No.</label>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="form-floating">
                        <input id="po-delivered_at" class="form-control" type="text" value="${ po.delivered_at ?? '--' }">
                        <label for="po-delivered_at">Delivery Date</label>
                    </div>
                </div>
                <div class="col-12">
                ...
                ${`<div class="col-12">
                    ...
                    <table class="table table-sm po-tbl-purchase_payment">
                        <thead>
                            <tr class="trth0">
                        ...
                        <tfoot class="text-end">
                            ...
                            <tr>
                                ...
                                <th></th>
                                <th>Total Paid:</th>
                                <th>${ number_format(po.total_paid, 2) }</th>
                            </tr>
                            <tr>
                                <th></th>
                                <th></th>
                                <th></th>
                                <th>Discount:</th>
                                <th>${ number_format(po.discount, 2) }</th>
                            </tr>
                            <tr>
                                <th></th>
                                <th></th>
                                <th></th>
                                <th>Balance:</th>
                                <th>${ number_format(po.total_balance, 2) }</th>
                            </tr>
                        </tfoot>
                    </table>
                </div>`}
            ...
            // foot
            `<button class="btn btn-success" type="button" onclick="purchase_repeat(${ po.vendor_id }, ${ id })"><i class="bi bi-arrow-counterclockwise me-2"></i>Repeat PO</button>`,
            // size
            `lg`
        );
    }
